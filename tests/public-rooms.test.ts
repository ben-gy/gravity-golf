 ...
 *   - a listed room stops being listed the moment the race starts, the room goes
 *     private, or the host leaves;
...
describe('roomAd — the one rule that decides a listing', () => {
  ...
  it('does not advertise a race in progress', () => {
...
describe('createListing', () => {
  it('announces only when the room is public', () => {
    ...
    expect(board.ads[0].note).toBe('Classic · 9 holes');
  });
  ...
  it('carries the live player count and the mode note', () => {
    ...
    const listing = createListing(board);
    listing.sync(LOBBY);
    listing.sync({ ...LOBBY, players: 3, note: 'Gauntlet · 9 holes' });
  ...
  it('gets off the board when the race starts', () => {
    const board = fakeBoard();
    const listing = createListing(board);
    listing.sync(LOBBY);
...
describe('room entry — browsing is opt-in', () => {
  ...
  it('offers no board at all when the game does not pass one', () => {
    // Not a style choice: no toggle and no Browse means there is nothing on this
    // screen that can join a mesh of strangers.
    createRoomEntry({ container, onSubmit: vi.fn() });
    expect(container.querySelector('.vis-chip')).toBeNull();
    expect(container.querySelector('.re-browse')).toBeNull();
    expect(container.querySelector('.re-note')).toBeNull();
  });
  ...
  it('lists host, note, players and whether a race is running', async () => {
    const board = fakeBoard();
    entry(board);
    click('.re-browse');
    await vi.waitFor(() => expect(board.opens).toBe(1));
    board.push([
      { code: 'ABCD', host: 'Ann', players: 2, max: 6, playing: false, note: 'Classic · 9 holes', seenAt: 0 },
      { code: 'WXYZ', host: 'Bo', players: 4, max: 6, playing: true, note: 'Sprint · 3 holes', seenAt: 0 },
    ]);

    const rows = [...container.querySelectorAll('.re-room')];
    expect(rows).toHaveLength(2);
    expect(rows[0].textContent).toContain('Ann');
    expect(rows[0].textContent).toContain('Classic · 9 holes');
  ...
  it('joins a room off the list as a GUEST, never claiming host', async () => {
    ...
    click('.re-browse');
    await vi.waitFor(() => expect(board.opens).toBe(1));
    board.push([{ code: 'ABCD', host: 'Ann', players: 2, max: 6, playing: false, seenAt: 0 }]);
    click('.re-room');
  ...
  it('joins a typed code as a guest even with Public selected', () => {
    ...
    input.value = 'K7QP';
    container
      .querySelector<HTMLFormElement>('.re-join')!
