 ...
 * constructs an identical RNG from it, so course layouts are IDENTICAL on all
 * clients without syncing every value. Never use Math.random() for anything
 * gameplay peers must agree on (here: the generated course).
 */
